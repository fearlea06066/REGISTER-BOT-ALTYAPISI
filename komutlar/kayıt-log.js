...
exports.run = async (client, message, args) => {
  ...
  db.set(`kayıtlog_${message.guild.id}`, kanal.id)//kayıt kanal kurma
  
  const emb = new Discord.RichEmbed()
  .setAuthor(client.user.username, client.user.displayAvatarURL)
  .setDescription(`**:white_check_mark: Kayıt Log Kanalı başarıyla ${kanal} olarak ayarlandı!\n Kayıt Logları Buraya Düçecektir => ${kanal}\n Eğer kayıt sistemini kapatmak istersen \`Örnek: ${prefix}kayıt kapat\`**`)
  .setColor('BLACK')
...
exports.help = {
 name: 'kayıt-log',
 description: 'kayıt log kanalı Olunacak kanalı seçersiniz',
